...
async function getProduct(res, id = false) {
    try{
...
app.post('/rest-api/products/update/:id', (req, res) => {
    ...
    async function updateProduct() {
        try{
            await client.connect()
            await client.db('rest-api').collection('products').updateOne(
                {
                    _id: new ObjectId(id)
                },
                {
                    $set: {
                        product: productData.newProductName,
                        category: productData.newCategory,
                        price: productData.newPrice,
                        qty: productData.newQty
                    }
                }
            )
...
app.post('/rest-api/insert', (req, res) => {
    ...
    async function insertProduct() {
        try{
            await client.connect()
            await client.db('rest-api').collection('products').insertOne(
                {
